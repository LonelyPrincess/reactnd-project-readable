...
function PostListItem (props) {
  ...
  const scoreClassName = (post.voteScore > 0
    ? 'positive'
    : (post.voteScore < 0
      ? 'negative'
      : ''
  ...
  return (
    <article className="post">
      <div className={`score-box ${scoreClassName}`}>
      ...
      <div className="button-container">
        <div className="info">
          <button disabled>
            <i className="fa fa-tag"></i> {post.category}
          </button>
          <button disabled>
            <i className="fa fa-comments"></i> {post.commentCount} comments
          </button>
        </div>
        <div className="actions">
          <button onClick={() => actions.updatePostScore(post, 'upVote')}>
            <i className="fa fa-thumbs-o-up"></i> Upvote
          </button>
          <button onClick={() => actions.updatePostScore(post, 'downVote')}>
            <i className="fa fa-thumbs-o-down"></i> Downvote
          </button>
          <Link to={`/${post.category}/${post.id}/edit`}><button>
